...
#define DYN 0
#define TRASH_LIM 1
#define TRASH_LEN 2896 //63712 //2896 //63712
#define RATE_LIM 0
#define COMP_PRINT 0
#define COMPRESS 1
#define BUFFSIZE 84
...
int svpn_server_handle_thread(struct svpn_server* pvoid)
{
	...
	float elapsed;
	long long int acc = 0, acc2 = 0, acc3 =0, comp_time, acc_avg = 0, n_iter=0, total_acc=0;
	struct timeval tv;
	...
	{	
											...
											//_______________________________________________________________________________________
											    ...
											    *totallen=0;
												...
												while(sent < TRASH_LEN){
													...
													FD_ZERO(&fd_list3);
													FD_SET(conn_fd, &fd_list3);
													timeout.tv_sec=0;
													timeout.tv_usec=TIMEOUT_USEC;
													select(maxfd3, NULL, &fd_list3, NULL, &timeout);
													if(ret < 0) {
														if(errno == EINTR)
															return 0;
														continue;
													...
													if(FD_ISSET(conn_fd, &fd_list3)){
														len=send(conn_fd, buffer + sent , TRASH_LEN - sent, MSG_NOSIGNAL);
														if(DEBUG_PRINT)
															printf("Came to line number %d \n", __LINE__);
														sent = sent + len;
														printf("\nSent_len = %d\n",sent);
		...
		{	if(DEBUG_PRINT)
			...
			while(sent_len < main_tlen + 8){
				...
				len = send(conn_fd, buffer + sent_len , main_tlen + 8 - sent_len, MSG_NOSIGNAL);
				if(len < 0)
				     printf("an error: %s\n", strerror(errno));
